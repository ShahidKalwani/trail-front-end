...
import { getRolesPermissions  } from '../pages/api/services'
import { useAuth } from '../hooks/auth'

import { useRouter } from 'next/router'
export default function Index({rolePermissionsChange}) {
    ...
    useEffect(() => {
        ...
        .then((response) => {
           if(response) {
            ...
            //   rolePermissionsChange(response.nav_items)
           }
        })
    },[])

    const navigateToPage = (el) => {
       router.push('http://localhost:3000/'+el.route)
    }
            ...
            <div className="w-64 absolute sm:relative bg-gray-800 shadow md:h-full flex-col justify-between hidden sm:flex">
                <div className="px-8">
                    ...
                    <ul className="mt-12">
                        {rolePermissions &&
                            <>
                                {rolePermissions.map(function(el, i){
                                    
                                    return <li className="flex w-full justify-between text-gray-300 hover:text-gray-500 cursor-pointer items-center mb-6" onClick={() => navigateToPage(el)}>
                        ...
                        <li className="flex w-full justify-between text-gray-300 hover:text-gray-500 cursor-pointer items-center mb-6" onClick={logout}>
                                            <div className="flex items-center">
                                                <svg xmlns="http://www.w3.org/2000/svg" className="icon icon-tabler icon-tabler-grid" width={18} height={18} viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" fill="none" strokeLinecap="round" strokeLinejoin="round">
                                                    <path stroke="none" d="M0 0h24v24H0z" />
                                                    <rect x={4} y={4} width={6} height={6} rx={1} />
